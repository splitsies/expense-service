...
export const main = middyfy(
    SplitsiesFunctionHandlerFactory.create<never, void | string>(
        logger,
        async (event) => {
            const expenseId = event.pathParameters.expenseId;
            const childExpenseId = event.pathParameters.childExpenseId;
            const userId = event.requestContext.authorizer.userId;

            await expenseService.removeExpenseFromGroup(expenseId, childExpenseId, userId);
