...
export const main = middyfy(
    SplitsiesFunctionHandlerFactory.create<typeof schema, string[] | string>(
        logger,
        async (event) => {
            const expenseId = event.pathParameters.expenseId;
            const userId = event.requestContext.authorizer.userId;

            const result = await expenseService.getUsersForExpense(expenseId);
            if (!result || !result.includes(userId)) {
